...
const filteredApplications = applications.filter(app => {
  // Use ps_status instead of status for consistency
  const status = (app.ps_status || '').toLowerCase();
  const filter = (filterStatus || '').toLowerCase();

  // Status filter
  let statusMatch = false;
  if (filter === 'all' || filter === 'selectstatus') statusMatch = true;
  else if (filter === 'pending') statusMatch = status.includes('pending');
  else if (filter === 'inprogress') statusMatch = status.includes('in progress') || status.includes('inprogress');
  else if (filter === 'returned' || filter === 'return') statusMatch = status.includes('returned') || status.includes('return');
  else if (filter === 'under review') statusMatch = status.includes('under review');
...
const isReportSubmitted = (appId) =>
  ...
  const handleSendToACP = async (appId) => {
    // Check if report is submitted before allowing to send to ACP
    if (!isReportSubmitted(appId)) {
      alert("Please submit the validation report first before sending to ACP.");
      return;
    }

    const confirmSend = window.confirm(
      "Are you sure you want to send this application to ACP?"
    );
    ...
    try {
      ...
      if (data.status) {
        alert("Application sent to ACP successfully!");
        // Optionally refresh the applications list or update the status
        // You might want to refetch the applications here
      } else {
        alert("Failed to send application to ACP. Please try again.");
      }
    } catch (error) {
      console.error("Failed to send application to ACP", error);
                  ...
                  <table className="table">
                    ...
                    <tbody>
                      {paginatedApplications.map((app, idx) => (
                        ...
                        <td>
                         <button
                              className={`btn btn-sm text-white ${isReportSubmitted(app.id) ? 'btn-info' : 'btn-secondary'}`}
                              onClick={() => handleSendToACP(app.id)}
                              disabled={!isReportSubmitted(app.id)}
                              title={!isReportSubmitted(app.id) ? "Submit validation report first" : "Send to ACP"}
                            >
